...
/* Initiate OV7670's CLK before setting */
...
/* Close OV7670's external CLK when using internal CLK */
...
void OV7670_GPIO_CONTRL_CONFIG(void)
{
    ...
    GPIO_InitTypeDef GPIO_InitStructure;
    __HAL_RCC_GPIOA_CLK_ENABLE();
    GPIO_InitStructure.Pin = LCD_HREF_BIT|LCD_VSYNC_BIT|LCD_PCLK_BIT;
    GPIO_InitStructure.Speed = GPIO_SPEED_HIGH; 
    GPIO_InitStructure.Mode = GPIO_MODE_INPUT; 
    HAL_GPIO_Init(GPIOC, &GPIO_InitStructure);
...
unsigned char wrOV7670Reg(unsigned char regID, unsigned char regDat)
{
	...
	uint8_t arr[2] = {regID, regDat};
	
	sta = HAL_I2C_Master_Transmit(&heval_I2c, 0x42, arr, 2, 100);
	if (HAL_OK != sta)
	{
		BSP_LED_Toggle(LED3);
	}
...
{
	unsigned char arr[2] = {regID, regDat[0]};
	HAL_I2C_Master_Transmit(&heval_I2c, 0x42, arr, 1, 100);
	HAL_I2C_Master_Receive(&heval_I2c, 0x43, arr, 1, 100);
	return 1;
...
//	wrOV7670Reg(0xa1, 0x03);//0x0b,
	...
	wrOV7670Reg(0x6b, 0x4a);   //PLL原来0x00	  pll控制	重要设置0x4a
...
unsigned char OV7670_init(void)
{
  ...
  //     CLK_init_ON();
	...
	OV7670_config_window(184,12,240,240);// set 240*320
...
void set_Cmos7670reg(void)
{
	...
	wrOV7670Reg(0x41, 0x18);
	wrOV7670Reg(0x3f, 0x00);
	wrOV7670Reg(0x75, 0x0F);
	wrOV7670Reg(0x76, 0xeF);
